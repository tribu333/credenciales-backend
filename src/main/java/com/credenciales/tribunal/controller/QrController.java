...
public class QrController {
    ...
    @Operation(summary = "Generar QR para personal", description = "Genera un nuevo código QR para un personal basado en su carnet de identidad")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "QR generado exitosamente", content = @Content(schema = @Schema(implementation = QrResponseDTO.class))),
    ...
    public ResponseEntity<QrResponseDTO> obtenerQrPorId(
            @Parameter(description = "ID del QR", required = true, example = "1") @PathVariable Long id) {
        return ResponseEntity.ok(qrService.obtenerQrPorId(id));
    }

    @Operation(summary = "Obtener QR por código", description = "Obtiene la información de un QR por su código único")
    @GetMapping("/codigo/{codigo}")
    public ResponseEntity<QrResponseDTO> obtenerQrPorCodigo(
            @Parameter(description = "Código del QR", required = true, example = "QR-1234567-20260215-ABC123") @PathVariable String codigo) {
        return ResponseEntity.ok(qrService.obtenerQrPorCodigo(codigo));
    }

    @Operation(summary = "Obtener QR por personal", description = "Obtiene el QR asignado a un personal específico")
    ...
    @Operation(summary = "Listar QR libres", description = "Obtiene todos los QR disponibles (estado LIBRE)")
    @GetMapping("/libres")
    public ResponseEntity<List<QrResponseDTO>> listarQrsLibres() {
        return ResponseEntity.ok(qrService.listarQrsLibres());
    }

    @Operation(summary = "Listar QR por tipo", description = "Obtiene todos los QR libres de un tipo específico")
    @GetMapping("/tipo/{tipo}")
    public ResponseEntity<List<QrResponseDTO>> listarQrsPorTipo(
            @Parameter(description = "Tipo de QR", required = true, example = "PERSONAL") @PathVariable TipoQr tipo) {
        return ResponseEntity.ok(qrService.listarQrsPorTipo(tipo));
    }

    @Operation(summary = "Asignar QR a personal", description = "Asigna un QR existente a un personal")
    ...
    @Operation(summary = "Liberar QR", description = "Libera un QR asignado, dejándolo disponible para otro personal")
    ...
    @Operation(summary = "Inactivar QR", description = "Inactiva un QR, dejándolo fuera de uso")
