...
public class PersonalServiceImpl implements PersonalService {
	...
	public VerificacionResponseDTO solicitarCodigoVerificacion(VerificacionEmailRequestDTO request) {
		...
		log.info("Solicitando código para email: {}, CI: {}", request.getCorreo(),
				request.getCarnetIdentidad());

		// Validar si el correo ya existe en personal activo
		if (existeCorreoActivo(request.getCorreo())) {
			throw new BusinessException(
					"El correo electrónico ya está registrado por otro personal activo");
		}
		...
		log.info("Código de verificación enviado a: {} para CI: {}", request.getCorreo(),
				request.getCarnetIdentidad());
	...
	public boolean verificarCodigo(VerificacionCodigoRequestDTO request) {
		VerificacionEmail verificacion = verificacionEmailRepository
				...
				.orElseThrow(() -> new BusinessException(
						"No hay código de verificación pendiente para este email"));
	...
	private PersonalCompletoDTO crearNuevoPersonal(PersonalCreateDTO registroDTO) {
		...
		Personal personal = Personal.builder()
				...
				.accesoComputo(registroDTO.getAccesoComputo() != null ? registroDTO.getAccesoComputo()
						: false)
		...
		if (registroDTO.getTipo() == TipoPersonal.PLANTA) {
			...
			if (registroDTO.getCargoID() == null) {
				throw new BusinessException(
						"Para personal de PLANTA debe especificar un cargo (cargoID)");
			}
			...
			if (registroDTO.getCargoID() == null) {
				throw new BusinessException(
						"Para personal EVENTUAL debe especificar un cargo de proceso (cargoID)");
			}
	...
	private PersonalCompletoDTO actualizarPersonalExistente(
		...
		personalExistente
				.setAccesoComputo(
						registroDTO.getAccesoComputo() != null ? registroDTO.getAccesoComputo()
								: false);
	...
	private void registrarCargoPlanta(Personal personal, Long cargoId) {
		Cargo cargo = cargoRepository.findById(cargoId)
				.orElseThrow(() -> new ResourceNotFoundException(
						"Cargo no encontrado con ID: " + cargoId));
		...
		return personalRepository.findByCorreo(correo)
				.map(personal -> {
					...
					return !estado.equals(EstadoPersonal.CREDENCIAL_DEVUELTO.getNombre()) &&
							!estado.equals(EstadoPersonal.PERSONAL_INACTIVO_PROCESO_TERMINADO
									.getNombre())
							&&
							!estado.equals(EstadoPersonal.INACTIVO_POR_RENUNCIA
									.getNombre());
				})
				.orElse(false);
	}
		...
		return personalRepository.findByCarnetIdentidad(carnetIdentidad)
				.map(personal -> {
					...
					if (estado.equals(EstadoPersonal.CREDENCIAL_ENTREGADO.getNombre()) ||
							...
							estado.equals(EstadoPersonal.PERSONAL_CON_ACCESO_A_COMPUTO
									.getNombre())) {
					...
					if (estado.equals(
							EstadoPersonal.PERSONAL_INACTIVO_PROCESO_TERMINADO.getNombre())
							||
							estado.equals(EstadoPersonal.INACTIVO_POR_RENUNCIA
									.getNombre())) {
	...
	public PersonalCompletoDTO obtenerPersonalPorCarnet(String carnetIdentidad) {
		Personal personal = personalRepository.findByCarnetIdentidad(carnetIdentidad)
				.orElseThrow(() -> new ResourceNotFoundException(
						"Personal no encontrado con CI: " + carnetIdentidad));
		return mapToCompletoDTO(personal);
	}
	...
	public PersonalCompletoDTO obtenerPersonalPorCorreo(String correo) {
		Personal personal = personalRepository.findByCorreo(correo)
				.orElseThrow(() -> new ResourceNotFoundException(
						"Personal no encontrado con correo: " + correo));
		return mapToCompletoDTO(personal);
	}
	...
	public PersonalCompletoDTO actualizarPersonal(Long id, PersonalCreateDTO actualizacionDTO) {
		...
		personal.setAccesoComputo(
				actualizacionDTO.getAccesoComputo() != null ? actualizacionDTO.getAccesoComputo()
						: false);
	...
	@Override
	public PersonalCompletoDTO actualizarPersonalExistenteAdmin(Long id,
			PersonalActualizacionDTO actualizacionDTO) {
		Personal personal = personalRepository.findById(id)
				.orElseThrow(() -> new ResourceNotFoundException(
						"Personal no encontrado con ID: " + id));
